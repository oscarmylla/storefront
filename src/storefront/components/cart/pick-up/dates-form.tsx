...
import { PickupDates, PickupLocation } from "@/storefront/types/zapiet";
...
const FormSchema = z.object({
  date: z.date({
    required_error: "Välj ett datum för upphämtning.",
  }),
});

export function PickupDatesForm({
  location,
  setDate,
  reset,
}: {
  ...
  const { data, isLoading, isValidating } = useSWR<PickupDates, Error>(
    `/api/zapiet/pickup/dates?locationId=${location.id}`,
    fetcher,
    {
  ...
  const disabledDates = data.disabled.reduce((acc: any, item: any) => {
    if (Array.isArray(item)) {
      const date = new Date(item[0], item[1] - 1, item[2]);
      acc.push(date);
    } else {
