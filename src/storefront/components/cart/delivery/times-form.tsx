...
export function DeliveryTimesForm({
  location,
  date,
  setTime,
}: {
  ...
  date: Date;
  setTime: Dispatch<SetStateAction<DeliveryTime | undefined>>;
}) {
  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
  });

  const { data, isLoading, isValidating } = useSWR<DeliveryTime[], Error>(
    `/api/zapiet/delivery/times?locationId=${location.id}&date=${format(date, "yyyy-MM-dd")}`,
    fetcher,
    {
  ...
  const selectedTime = data.find(
    (time) => time.id.toString() === form.getValues("time")
  );
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-2/3 space-y-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Leveranstid</FormLabel>
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger className="w-[240px]">
                    <SelectValue placeholder="Välj leveranstid">
              ...
              <FormDescription>Leveranstid för din order.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
